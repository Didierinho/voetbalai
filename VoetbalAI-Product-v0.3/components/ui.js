...
export function Header(){
 return <><div className="demo">PRODUCTDEMO v0.3 <span>•</span> fictieve beeldidentiteit • AI-redactie voorbereid</div>
...
export function Thumb({a, large=false}){
 ...
 return <div className={`thumb editorial ${a.theme} ${a.kind||""} ${large?"large":""}`}>
   <div className="texture"></div><div className="beam beam1"></div><div className="beam beam2"></div>
   <div className="stadiumBowl"></div><div className="crowdDots"></div>
   <div className="editorialTag">{a.category}</div>
   <Crest club={a.club}/>
   {a.kind==="transfer" && <><div className="transferArrow">→</div><div className="mysteryCrest"><b>?</b><small>NIEUWE CLUB</small></div></>}
   {a.kind!=="transfer" && <div className="storySymbol">{symbol}</div>}
   <div className="thumbCaption"><b>{a.club}</b><span>VOETBALAI VISUAL</span></div>
 </div>
}
...
export function Footer(){return <footer><div className="wrap footerGrid"><div><b>VOETBALAI</b><p>AI-native voetbalnieuws • productdemo v0.3</p></div><Link href="/redactie">Open redactieomgeving →</Link></div></footer>}
